...
def plot_forex_vs_index(forex_pair, dollar_index, start_date, end_date, interval='1h'):
    try:
        ...
        # Plot the standardized forex data on the primary y-axis
        color = 'tab:blue'
        ax1.set_xlabel('Date')
        ax1.set_ylabel(forex_pair + ' (Standardized)', color=color)
        ax1.plot(data_standardized.index, data_standardized[forex_pair], label=forex_pair, color=color)
        ax1.tick_params(axis='y', labelcolor=color)

        # Instantiate a second y-axis sharing the same x-axis
        ax2 = ax1.twinx()
        color = 'tab:red'
        ax2.set_ylabel(dollar_index + ' (Standardized)', color=color)
        ax2.plot(data_standardized.index, data_standardized[dollar_index], label=dollar_index, color=color)
        ax2.tick_params(axis='y', labelcolor=color)

        # Add a title
        plt.title(f'{forex_pair} vs {dollar_index} (Standardized)')
