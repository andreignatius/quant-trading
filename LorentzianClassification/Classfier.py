...
class LorentzianClassification:
    ...
    def plot(self):
        ...
        sub_plots = [
            mpf.make_addplot(self.yhat1[:len], ylabel="Kernel Regression Estimate", color='blue', secondary_y = False),
            mpf.make_addplot(self.yhat2[:len], ylabel="yhat2", color='gray', secondary_y = False),
            mpf.make_addplot(self.df["startLongTrade"], ylabel="startLongTrade", color='green', type='scatter', markersize=120, marker='^', secondary_y = False),
            mpf.make_addplot(self.df["startShortTrade"], ylabel="startShortTrade", color='red', type='scatter', markersize=120, marker='v', secondary_y = False),
        ]
        ...
        fig, axlist = mpf.plot(self.df[['Open', 'High', 'Low', 'Close']].head(len), type='candle', style=s, addplot=sub_plots, figsize=(40,20) ,returnfig=True)
